...
void OLED_ShowNum(uint8_t x, uint8_t y, uint32_t Number, uint8_t Length)
{
	...
	for (i = 0; i < Length; i++)							
	{
		OledShowChar(x + 8,y, Number / OLED_Pow(10, Length - i - 1) % 10 + '0',1);
	}
}
*/
...
{
    uint8_t i;
    uint8_t newX = x; // New x position for each digit

    for (i = 0; i < Length; i++)
    {
        OledShowChar(newX, y, Number / OLED_Pow(10, Length - i - 1) % 10 + '0', 1);
        newX += 8; // Increment x by 8 for the next digit
    }
}