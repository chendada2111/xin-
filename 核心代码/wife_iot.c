...
#define CONFIG_WIFI_SSID "孩子玩会吧" // 修改为自己的WiFi 热点账号

#define CONFIG_WIFI_PWD "meidong6" // 修改为自己的WiFi 热点密码

#define CONFIG_APP_SERVERIP "117.78.5.125"//标准版

#define CONFIG_APP_SERVERPORT "1883"

#define CONFIG_APP_DEVICEID "65e1a64071d845632afe4eec_0422" // 替换为注册设备后生成的deviceid
...
static void deal_report_msg(report_t *report)//需要上传数据，先拼接数据，然后通过mqtt上报数据
{
    ...
    service.service_property = &bmpp;
    service.nxt = NULL;

    

    bmpp.key = "BPM";
    bmpp.value = &report->bmp;
    bmpp.type = EN_OC_MQTT_PROFILE_VALUE_INT;
    bmpp.nxt = &led;
    ...
    oc_mqtt_profile_propertyreport(NULL, &service); //上报数据
    return;
}

// use this function to push all the message to the buffer
static int msg_rcv_callback(oc_mqtt_profile_msgrcv_t *msg)//接受命令并发送到队列中
{
    int ret = 0;
    char *buf;
    int buf_len;
...
///< COMMAND DEAL
#include <cJSON.h>
static void deal_light_cmd(cmd_t *cmd, cJSON *obj_root)//电灯命令
{
    cJSON *obj_paras;
    cJSON *obj_para;
    int cmdret;
...
static void deal_motor_cmd(cmd_t *cmd, cJSON *obj_root)//电机命令
{
    cJSON *obj_paras;
    cJSON *obj_para;
    int cmdret;
...
static void deal_cmd_msg(cmd_t *cmd)//总命令下发
{
    cJSON *obj_root;
    cJSON *obj_cmdname;
...
static int CloudMainTaskEntry(void)//连接华为iot平台
{
    app_msg_t *app_msg;
    uint32_t ret;
...
static int SensorTaskEntry(void)//传数据
{
   ...
   BPMVALUE=heartTask();
    ...
    while (1) {
        ...
        if (app_msg != NULL) {
            ...
            app_msg->msg.report.bmp=(int)BPMVALUE;//加一个传感数据
